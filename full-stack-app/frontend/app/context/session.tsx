...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const handleTokenChange = (newToken: string | null) => {
    if (newToken) {
      ...
      setIsLoading(false); // Set loading to false after token is set
      ...
      setIsLoading(false); // Set loading to false after token is cleared
    }
  };
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Set loading to false after checking for existing token
      setIsLoading(false);
    } else {
      // If not in browser, set loading to false
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <SessionContext.Provider value={{ user, token, login, logout, isAuthenticated, isLoading }}>
      {children}
